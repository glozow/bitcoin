...
void CMainSignals::TransactionAddedToMempool(const CTransactionRef& tx, const TxMempoolInfo& txinfo, uint64_t mempool_sequence) {
    auto event = [&txinfo, tx, mempool_sequence, this] {
        m_internals->Iterate([&](CValidationInterface& callbacks) { callbacks.TransactionAddedToMempool(tx, txinfo, mempool_sequence); });
    };
