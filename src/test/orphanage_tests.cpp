...
static CTransactionRef MakeLargeOrphan()
{
    ...
    for (unsigned int j = 0; j < tx.vin.size(); j++) {
        ...
        tx.vin[j].prevout.hash = Txid::FromUint256(GetRandHash());
...
BOOST_AUTO_TEST_CASE(DoS_mapOrphans)
{
    ...
    orphanage.LimitOrphans(40, DEFAULT_MAX_ORPHAN_TOTAL_SIZE, rng);
    BOOST_CHECK(orphanage.CountOrphans() <= 40);
    orphanage.LimitOrphans(10, DEFAULT_MAX_ORPHAN_TOTAL_SIZE, rng);
    BOOST_CHECK(orphanage.CountOrphans() <= 10);
    orphanage.LimitOrphans(0, DEFAULT_MAX_ORPHAN_TOTAL_SIZE, rng);
    ...
    orphanage.LimitOrphans(modified_max_count, DEFAULT_MAX_ORPHAN_TOTAL_SIZE, rng);
    ...
    orphanage.LimitOrphans(/*max_orphans=*/10, DEFAULT_MAX_ORPHAN_TOTAL_SIZE, rng);
...
BOOST_AUTO_TEST_CASE(process_block)
{
    ...
    for (uint32_t i{0}; i < num_outpoints; ++i) {
        ...
        outpoints.emplace_back(COutPoint{Txid::FromUint256(det_rand.rand256()), i});
    }
