...
static std::map<TxValidationResult, Behaviors> expected_behaviors{
    ...
    {TxValidationResult::TX_MISSING_INPUTS,          {/*txid_rejects*/0, /*wtxid_rejects*/0, /*txid_recon*/0, /*wtxid_recon*/0, /*keep*/0, /*txid_inv*/0, /*wtxid_inv*/0}},
...
BOOST_FIXTURE_TEST_CASE(tx_rejection_types, TestChain100Setup)
{
    ...
    for (const auto& [result, expected_behavior] : expected_behaviors) {
        ...
        const bool keep = txdownload_impl.MempoolRejectedTx(ptx, state);
        Behaviors actual_behavior{
            ...
            /*keep=*/keep,
            /*txid_inv=*/txdownload_impl.AlreadyHaveTx(GenTxid::Txid(txid), true),
            /*wtxid_inv=*/txdownload_impl.AlreadyHaveTx(GenTxid::Wtxid(wtxid), true),
        };
