...
BOOST_FIXTURE_TEST_CASE(packageifier_tests, TestChain100Setup)
{
    ...
    // SkipWithDescendants the 40th transaction. GetAncestorSet() for all of its descendants should return std::nullopt.
    ...
    }
    {
        ...
        BOOST_CHECK(PackageSorted(packageified.GetAncestorSet(child).value(), package_sorted));
    }
    {
        ...
        BOOST_CHECK(PackageSorted(packageified7.GetAncestorSet(child).value(), package_sorted));
        ...
        packageified5.AddFeeAndVsize(parent_med_feerate_add100->GetHash(), med_fee_amt + 100, GetVirtualTransactionSize(*parent_med_feerate_add100));
        ...
        BOOST_CHECK(PackageSorted(packageified5.GetAncestorSet(child).value(), package_sorted_with_skip));
...
BOOST_FIXTURE_TEST_CASE(package_submission_tests, TestChain100Setup)
{
    ...
    CheckPackageMempoolAcceptResult(package_unrelated, result_unrelated_submit, /*expect_valid=*/false, m_node.mempool.get());
...
BOOST_FIXTURE_TEST_CASE(package_cpfp_tests, TestChain100Setup)
{
    ...
    m_node.mempool->PrioritiseTransaction(tx_parent->GetHash(), child_value - coinbase_value);
    {
        ...
        BOOST_CHECK_EQUAL(submit_cpfp_deprio.m_tx_results.size(), package_cpfp.size());
    ...
    // Cheap package should fail for being too low fee.
    {
        ...
        BOOST_CHECK_EQUAL(submit_package_too_low.m_tx_results.size(), package_still_too_low.size());
    ...
    // The correct behavior is to accept Parent1 and Parent2, but not the child.
    {
        ...
        CheckPackageMempoolAcceptResult(package_parent_pfp, submit_parent_pfp, /*expect_valid=*/true, m_node.mempool.get());
        expected_pool_size += 2;
