...
BOOST_FIXTURE_TEST_CASE(linearization_tests, TestChain100Setup)
{
...

    {
        ...
        if (auto err_needs_reorder{CheckPackageMempoolAcceptResult(package_needs_reorder, result_needs_reorder, /*expect_valid=*/false, m_node.mempool.get())}) {
            ...
            std::vector<Wtxid> wtxids_top4{tx_grandparent1->GetWitnessHash(), tx_grandparent2->GetWitnessHash(), tx_parent1->GetWitnessHash(), tx_parent2->GetWitnessHash()};
            ...
            // ... then, grandparent2 + parent2 + grandparent1 + parent1
            BOOST_CHECK_EQUAL(result_needs_reorder.m_tx_results.at(tx_parent2->GetWitnessHash()).m_wtxids_fee_calculations->size(), 4);
            BOOST_CHECK_EQUAL(result_needs_reorder.m_tx_results.at(tx_parent1->GetWitnessHash()).m_wtxids_fee_calculations->size(), 4);

            BOOST_CHECK(result_needs_reorder.m_tx_results.at(tx_parent1->GetWitnessHash()).m_wtxids_fee_calculations.value() == wtxids_top4);
            BOOST_CHECK(result_needs_reorder.m_tx_results.at(tx_parent2->GetWitnessHash()).m_wtxids_fee_calculations.value() == wtxids_top4);
        }
    }
}
