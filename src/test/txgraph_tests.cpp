...
BOOST_AUTO_TEST_CASE(txgraph_trim_zigzag)
{
    ...
    const unsigned int num_bottom_tx = max_cluster_count - 1;
    ...
    const unsigned int num_top_tx = max_cluster_count;
    ...
    for (unsigned int i = 0; i < num_bottom_tx; ++i) {
        refs.push_back(graph->AddTransaction({(int64_t)(200-i), 100}));
    }
    ...
    BOOST_CHECK_EQUAL(graph->GetTransactionCount(), max_cluster_count * 2 - 1);
...
BOOST_AUTO_TEST_CASE(txgraph_trim_flower)
{
    ...
    for (unsigned int i = 0; i < num_top_tx; ++i) {
        refs.push_back(graph->AddTransaction({(int64_t)(500+i), 100}));
    }
