...
FUZZ_TARGET(tx_package_sort, .init = initialize_tx_pool)
{
    ...
    for (const auto& tx : txns) {
        ...
        if (no_package_deps && fuzzed_data_provider.ConsumeBool()) {
            ...
            package_sorter.RegisterInfo(tx, amount_fee, size);
        }
    }
    ...
    std::optional<FeePerWeight> last_feerate;
    bool accept_all{true};

    // 3. Subpackage validation
    auto chunk = package_sorter.GetCurrentSubpackage();
    while (chunk) {
        // Chunk feerates monotonously decrease
        Assert(!last_feerate || !(last_feerate.value() << chunk->second));
        last_feerate = chunk->second;
