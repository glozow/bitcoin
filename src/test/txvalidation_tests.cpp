...
BOOST_FIXTURE_TEST_CASE(version3_tests, RegTestingSetup)
{
    ...
    // Configuration where the tx has too many direct parents.
    {
        ...
        BOOST_CHECK(ApplyV3Rules(tx_v3_many_parents, ancestors).has_value());
        ancestors.clear();
    }
    ...
    }
    {
        ...
        BOOST_CHECK(ancestors.size() == 25);
        BOOST_CHECK(ancestors.size() + 1 > V3_ANCESTOR_LIMIT);
        BOOST_CHECK(ApplyV3Rules(tx_v3_many_generation, ancestors).has_value());
        ancestors.clear();
    }
    ...
    }
    {
        ...
        BOOST_CHECK(ApplyV3Rules(tx_v3_large_parents, ancestors).has_value());
        ancestors.clear();
    }
    ...
    many_inputs.push_back(COutPoint{mempool_tx_v3->GetHash(), 0});
    {
        ...
        auto v3_ancestors = GetV3Ancestors(ancestors);
        BOOST_CHECK(v3_ancestors.size() == 1);
        BOOST_CHECK(ApplyV3Rules(tx_v3_child_big, ancestors).has_value());
        ancestors.clear();
    }

    // Parent + child with v3 in the mempool.
    ...
    BOOST_CHECK(ancestors.size() == 1);
    BOOST_CHECK(ApplyV3Rules(tx_mempool_v3_child, ancestors) == std::nullopt);
    ...
    // A v3 transaction cannot have more than 1 descendant.
    {
        ...
        BOOST_CHECK(ancestors.size() == 1);
        BOOST_CHECK(ApplyV3Rules(tx_v3_child2, ancestors).has_value());
        ancestors.clear();
    }

    {
        ...
        BOOST_CHECK(ApplyV3Rules(tx_v3_grandchild, ancestors).has_value());
        ancestors.clear();
    }
}
