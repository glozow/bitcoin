...
public:
     ...
     * @returns true it peer is an announcer of wtxid after this call (including if it was already
     * an announcer), false if the tx doesn't exist. */
    ...
    std::vector<Wtxid> EraseForBlock(const CBlock& block) EXCLUSIVE_LOCKS_REQUIRED(!m_mutex);

    /** Limit the orphanage to the given maximum. Returns all expired entries. */
    std::vector<Wtxid> LimitOrphans(unsigned int max_orphans, unsigned int max_total_size = DEFAULT_MAX_ORPHAN_TOTAL_SIZE)
