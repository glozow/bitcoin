...
protected:
    ...
    struct PeerOrphanInfo {
         ...
         */
        [[nodiscard]] std::optional<OrphanMap::iterator> RemoveIterAt(size_t list_pos) {
            std::optional<OrphanMap::iterator> ret;
            if (list_pos + 1 != m_iter_list.size()) {
                // Unless we're deleting the last entry in m_iter_list, move the last entry to the
                // position we're deleting.
                auto it_last = m_iter_list.back();
                m_iter_list[list_pos] = it_last;
                ret = it_last;
            }
            m_iter_list.pop_back();
            return ret;
        }
    };
