...
std::vector<CTransactionRef> AncestorPackage::GetAncestorSet(const CTransactionRef& tx)
{
    auto ancestor_set = ancestor_subsets.find(tx->GetHash());
    Assume(ancestor_set != ancestor_subsets.end());
    ...
    for (const auto& txid : ancestor_set->second) {
        ...
        Assume(it != txid_to_tx.end());
        if (excluded_txns.find(txid) == excluded_txns.end()) {
            result.push_back(it->second);
        }
    }
    std::sort(result.begin(), result.end(), [this](const CTransactionRef& a, const CTransactionRef& b) -> bool {
        auto a_ancestors = ancestor_subsets.find(a->GetHash());
        auto b_ancestors = ancestor_subsets.find(b->GetHash());
        Assume(a_ancestors != ancestor_subsets.end());
        Assume(b_ancestors != ancestor_subsets.end());
