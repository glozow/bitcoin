...
void AncestorPackage::visit(const CTransactionRef& curr_tx)
{
    ...
    auto& entry = txid_to_entry.at(curr_txid);
    ...
    for (const auto& input : curr_tx->vin) {
        ...
        if (txid_to_entry.count(parent_txid) == 0) continue;
        auto parent_tx = txid_to_entry.at(parent_txid).tx;
        if (txid_to_entry.at(parent_txid).ancestor_subset.empty()) {
            visit(parent_tx);
        }
        auto parent_ancestor_set = txid_to_entry.at(parent_txid).ancestor_subset;
...
AncestorPackage::AncestorPackage(const Package& txns_in)
{
    ...
    // Populate txid_to_entry for quick lookup
    std::transform(txns_in.cbegin(), txns_in.cend(), std::inserter(txid_to_entry, txid_to_entry.end()),
    ...
    while (i < txns_in.size()) {
        ...
        if (txid_to_entry.at(tx->GetHash()).ancestor_subset.empty()) visit(tx);
        Assume(!txid_to_entry.at(tx->GetHash()).ancestor_subset.empty());
        ++i;
    }
    txns = txns_in;
    ...
    std::sort(txns.begin(), txns.end(), [&](const CTransactionRef& a, const CTransactionRef& b) -> bool {
        return txid_to_entry.at(a->GetHash()) < txid_to_entry.at(b->GetHash());
    });
    Assume(IsSorted(txns));
    is_ancestor_package = txid_to_entry.at(txns.back()->GetHash()).ancestor_subset.size() == txns.size();
    // Now populate the descendant caches
    for (const auto& [txid, entry] : txid_to_entry) {
        for (const auto& anc_txid : entry.ancestor_subset) {
            txid_to_entry.at(anc_txid).descendant_subset.insert(txid);
        }
    }
}
...
{
    Package result;
    for (const auto& [_, entry] : txid_to_entry) {
...
{
    if (txid_to_entry.count(tx->GetHash()) == 0) return std::nullopt;
    const auto& entry = txid_to_entry.find(tx->GetHash())->second;
    ...
    for (const auto& txid : entry.ancestor_subset) {
        const auto& anc_entry = txid_to_entry.at(txid);
...
{
    if (txid_to_entry.count(transaction->GetHash()) == 0) return;
    txid_to_entry.at(transaction->GetHash()).skip = true;
}
void AncestorPackage::SkipWithDescendants(const CTransactionRef& transaction)
{
    if (txid_to_entry.count(transaction->GetHash()) == 0) return;
    txid_to_entry.at(transaction->GetHash()).skip = true;
    for (const auto& descendant_txid : txid_to_entry.at(transaction->GetHash()).descendant_subset) {
        txid_to_entry.at(descendant_txid).skip = true;
        txid_to_entry.at(descendant_txid).dangles = true;
    }
}

void AncestorPackage::AddFeeAndVsize(const uint256& txid, CAmount fee, int64_t vsize)
{
    if (txid_to_entry.count(txid) == 0) return;
    txid_to_entry.at(txid).fee = fee;
    txid_to_entry.at(txid).vsize = vsize;
}

bool AncestorPackage::LinearizeWithFees()
{
    if (!is_ancestor_package) return false;
    // All fee and vsize information for non-skipped transactions must be available, otherwise linearization cannot be done.
    if (!std::all_of(txid_to_entry.cbegin(), txid_to_entry.cend(),
    ...
    for (auto& [_, entry] : txid_to_entry) entry.mining_sequence = std::nullopt;
    ...
    for (const auto& [txid, entry] : txid_to_entry) {
        ...
        CAmount ancestor_subset_fees = std::accumulate(filtered_ancestor_subset->cbegin(), filtered_ancestor_subset->cend(),
            CAmount{0}, [&](CAmount sum, const auto& anc) { return sum + *txid_to_entry.at(anc->GetHash()).fee; });
        int64_t ancestor_subset_vsize = std::accumulate(filtered_ancestor_subset->cbegin(), filtered_ancestor_subset->cend(),
            int64_t{0}, [&](int64_t sum, const auto& anc) { return sum + *txid_to_entry.at(anc->GetHash()).vsize; });
    ...
    for (const auto& [txid, mining_sequence] : miniminer.Linearize()) {
        txid_to_entry.at(txid).mining_sequence = mining_sequence;
    }
    // Sort again, this time using mining score.
    std::sort(txns.begin(), txns.end(), [&](const CTransactionRef& a, const CTransactionRef& b) -> bool {
        return txid_to_entry.at(a->GetHash()) < txid_to_entry.at(b->GetHash());
    });
    Assume(std::all_of(txid_to_entry.cbegin(), txid_to_entry.cend(), [](const auto& entry) {
