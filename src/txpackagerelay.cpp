    ...
    std::vector<GenTxid> GetOrphanRequests(NodeId nodeid)
    {
        ...
        for (const auto& gtxid : tracker_requestable) {
            if (gtxid.IsWtxid()) {
                ...
                // "Convert" this to a wtxid-type GenTxid, because the hash is wtxid of the orphan.
                const auto [ptx, from_peer] = orphanage_ref.GetTx(GenTxid::Wtxid(gtxid.GetHash()));
                if (!ptx) {
