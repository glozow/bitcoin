...
private:
    ...
    void ProcessInvalidTx(NodeId nodeid, const CTransactionRef& tx, const TxValidationState& result,
        ...
        EXCLUSIVE_LOCKS_REQUIRED(!m_peer_mutex, g_msgproc_mutex, cs_main, m_tx_download_mutex);
                                     ...
                                     const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (msg_type == NetMsgType::TX) {
        ...
        {
            const auto& [parent_txids, added] = m_txdownloadman.MaybeAddNewOrphan(ptx, pfrom.GetId());
            for (const auto& parent_txid : parent_txids) {
                AddKnownTx(*peer, parent_txid);
            }
            if (added) AddToCompactExtraTransactions(ptx);
        }
        if (state.IsInvalid()) {
