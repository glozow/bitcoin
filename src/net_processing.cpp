...
void PeerManagerImpl::AddOrphanResolutionCandidates(const CTransactionRef& orphan, NodeId originator)
{
    ...
    // We may prefer to resolve this orphan with certain types of peers over others (e.g.
    // outbounds). Also, if the first request fails, we should try again.  Get all peers that
    // announced this transaction and prioritize accordingly...
    ...
    if (msg_type == NetMsgType::INV) {
        ...
        for (CInv& inv : vInv) {
            ...
            if (inv.IsMsgBlk()) {
                ...
                } else if (inv.IsMsgWtx() && m_txpackagetracker->OrphanageHaveTx(gtxid) &&
                           !m_chainman.ActiveChainstate().IsInitialBlockDownload()) {
...
bool PeerManagerImpl::SendMessages(CNode* pto)
{
...

    {
        ...
        for (const auto& gtxid : m_txpackagetracker->GetOrphanRequests(pto->GetId(), current_time)) {
            if (!AlreadyHaveTx(gtxid)) {
                AddKnownTx(*peer, gtxid.GetHash());
