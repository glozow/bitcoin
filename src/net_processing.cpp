...
void PeerManagerImpl::AddOrphanResolutionCandidate(NodeId nodeid, const uint256& orphan_wtxid, std::chrono::microseconds current_time)
{
    ...
    if (!relay_permissions) {
        ...
        if (packages_in_flight >= node::MAX_IN_FLIGHT_PACKAGES ||
      ...
      m_enable_package_relay{enable_package_relay}
{
    ...
    m_txpackagetracker = std::make_unique<node::TxPackageTracker>(m_orphanage);
}
    ...
    if (msg_type == NetMsgType::GETPKGTXNS) {
...

        {
            ...
            for (const auto& wtxid : txns_requested) {
                auto ptx = FindTxForGetData(*peer, GenTxid::Wtxid(wtxid), mempool_req, now);
                if (ptx) {
