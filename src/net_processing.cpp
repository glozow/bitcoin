...
PeerManagerImpl::PeerManagerImpl(CConnman& connman, AddrMan& addrman,
    ...
    : m_chainparams(chainman.GetParams()),
      ...
      m_chainman(chainman),
      m_mempool(pool),
      m_txdownloadman(node::TxDownloadManager::Options{opts.max_orphan_txs, pool}),
      m_opts{opts}
{
...
void PeerManagerImpl::BlockConnected(const std::shared_ptr<const CBlock>& pblock, const CBlockIndex* pindex)
{
    ...
    m_txdownloadman.BlockConnected(*pblock.get(), m_chainman.ActiveChain().Tip()->GetBlockHash());
    m_last_tip_update = GetTime<std::chrono::seconds>();
                                     ...
                                     const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (msg_type == NetMsgType::TX) {
        ...
        m_txdownloadman.ReceivedTx(pfrom.GetId(), txid);
        if (tx.HasWitness()) m_txdownloadman.ReceivedTx(pfrom.GetId(), wtxid);
        ...
        if (m_txdownloadman.AlreadyHaveTx(GenTxid::Wtxid(wtxid))) {
            ...
            // If a tx has been detected by m_recent_rejects, it is ignored. Because we haven't
        ...
        if (result.m_result_type == MempoolAcceptResult::ResultType::VALID) {
            ...
            if (should_process_orphan) {
                ...
                unique_parents.erase(std::remove_if(unique_parents.begin(), unique_parents.end(),
                    ...
                    { return m_txdownloadman.AlreadyHaveTx(GenTxid::Txid(txid)); }),
                    unique_parents.end());

                for (const uint256& parent_txid : unique_parents) {
                    if (!m_txdownloadman.AlreadyHaveTx(GenTxid::Txid(parent_txid))) {
                        AddKnownTx(*peer, parent_txid);
                    }
                }
                if (m_txdownloadman.NewOrphanTx(ptx, unique_parents, pfrom.GetId(), current_time)) {
        ...
        if (vInv.size() <= node::MAX_PEER_TX_ANNOUNCEMENTS + MAX_BLOCKS_IN_TRANSIT_PER_PEER) {
            ...
            for (CInv &inv : vInv) {
                if (inv.IsGenTxMsg()) {
                    ...
                    m_txdownloadman.ReceivedTx(pfrom.GetId(), inv.hash);
                }
            }
        }
        return;
