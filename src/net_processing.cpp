...
void PeerManagerImpl::FinalizeNode(const CNode& node)
{
    ...
    m_txpackagetracker->EraseOrphanForPeer(nodeid);
...
{
    m_txpackagetracker->HandleNewBlock(*pblock);
    m_last_tip_update = GetTime<std::chrono::seconds>();
                                     ...
                                     const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (msg_type == NetMsgType::ANCPKGINFO) {
        ...
        for (const auto& wtxid : package_wtxids) {
            if (m_txpackagetracker->OrphanageHaveTx(GenTxid::Wtxid(wtxid))) {
                total_orphan_size += m_txpackagetracker->OrphanageGetTxSize(wtxid);
