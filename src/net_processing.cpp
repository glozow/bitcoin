...
private:
    ...
    bool ProcessInvalidTx(const CTransactionRef& tx, const TxValidationState& state)
    ...
    */
    ...
    /** Protects tx download including TxRequest, TxDownloadManager, rejection filter. */
...
void PeerManagerImpl::InitializeNode(CNode& node, ServiceFlags our_services)
{
    NodeId nodeid = node.GetId();
    {
        ...
        assert(m_txdownloadman.TxRequestCount(nodeid) == 0);
    }
...
void PeerManagerImpl::FinalizeNode(const CNode& node)
{
    ...
    if (m_node_states.empty()) {
        ...
        assert(m_wtxid_relay_peers == 0);
        LOCK(m_tx_download_mutex);
        assert(m_txdownloadman.TxRequestSize() == 0);
        assert(m_txdownloadman.OrphanageSize() == 0);
    }
    } // cs_main
...
void PeerManagerImpl::BlockDisconnected(const std::shared_ptr<const CBlock> &block, const CBlockIndex* pindex)
{
    ...
    LOCK(m_tx_download_mutex);
    m_txdownloadman.RecentConfirmedReset();
}
...
bool PeerManagerImpl::ProcessInvalidTx(const CTransactionRef& tx, const TxValidationState& state)
{
    AssertLockHeld(m_tx_download_mutex);
...
bool PeerManagerImpl::ProcessOrphanTx(Peer& peer)
{
    ...
    while (CTransactionRef porphanTx = m_txdownloadman.OrphanageGetTxToReconsider(peer.m_id)) {
        ...
        if (result.m_result_type == MempoolAcceptResult::ResultType::VALID) {
            ...
            // orphan resolution requests when a transaction is missing inputs.
            ProcessInvalidTx(porphanTx, state);
            return true;
        }
    }
                                     ...
                                     const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (msg_type == NetMsgType::VERACK) {
        ...
        if (auto tx_relay = peer->GetTxRelay()) {
            ...
            m_txdownloadman.ConnectedPeer(pfrom.GetId(), node::TxDownloadManager::ConnectionInfo {
    ...
    if (msg_type == NetMsgType::TX) {
        ...
        if (AlreadyHaveTx(GenTxid::Wtxid(wtxid))) {
            ...
            // If a tx has been detected by m_recent_rejects (or another filter
            // queried in AlreadyHaveTx), the tx is ignored. Because we haven't
        ...
        if (result.m_result_type == MempoolAcceptResult::ResultType::VALID) {
            ...
            bool should_process_orphan = ProcessInvalidTx(ptx, state);
            if (should_process_orphan) {
                ...
                unique_parents.erase(std::remove_if(unique_parents.begin(), unique_parents.end(),
                    [&](const auto& txid) EXCLUSIVE_LOCKS_REQUIRED(::cs_main, m_tx_download_mutex)
                    { return AlreadyHaveTx(GenTxid::Txid(txid)); }), unique_parents.end());
                ...
                if (!unique_parents.empty() &&
                    m_txdownloadman.NewOrphanTx(ptx, unique_parents, pfrom.GetId(), current_time)) {
...
bool PeerManagerImpl::ProcessMessages(CNode* pfrom, std::atomic<bool>& interruptMsgProc)
{
    ...
    try {
        ...
        LOCK(m_tx_download_mutex);
        if (m_txdownloadman.OrphanageHaveTxToReconsider(peer->m_id)) fMoreWork = true;
...
bool PeerManagerImpl::SendMessages(CNode* pto)
{
...

    {
        ...
        auto requestable = m_txdownloadman.TxRequestGetRequestable(pto->GetId(), current_time, &expired);
        ...
        for (const GenTxid& gtxid : requestable) {
            if (!AlreadyHaveTx(gtxid)) {
                ...
                m_txdownloadman.TxRequestRequestedTx(pto->GetId(), gtxid.GetHash(), current_time + GETDATA_TX_INTERVAL);
            } else {
                // We have already seen this transaction, no need to download. This is just a belt-and-suspenders, as
                // this should already be called whenever a transaction becomes AlreadyHaveTx().
                m_txdownloadman.TxRequestForgetTxHash(gtxid.GetHash());
            }
        }
