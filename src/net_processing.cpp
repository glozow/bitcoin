...
void PeerManagerImpl::AddOrphanResolutionCandidate(NodeId peer_originator, const std::pair<uint256, CTransactionRef>& orphan)
{
    ...
    for (const auto nodeid : candidate_peers) {
        ...
        //  TODO: bypass if the peer has Relay permissions
...
bool PeerManagerImpl::ProcessOrphanTx(Peer& peer)
{
    ...
    while (CTransactionRef porphanTx = m_txpackagetracker->GetTxToReconsider(peer.m_id)) {
        ...
        if (result.m_result_type == MempoolAcceptResult::ResultType::VALID) {
            ...
            m_txpackagetracker->TransactionAccepted(porphanTx);
            ...
            m_txpackagetracker->TransactionRejected(porphanTx->GetWitnessHash());
            return true;
        }
    }
                                     ...
                                     const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (msg_type == NetMsgType::TX) {
        ...
        if (result.m_result_type == MempoolAcceptResult::ResultType::VALID) {
            ...
            RelayTransaction(tx.GetHash(), tx.GetWitnessHash());
            m_txpackagetracker->TransactionAccepted(ptx);
...

    {
        ...
        for (const auto& gtxid : requestable_orphans) {
            if (AlreadyHaveTx(gtxid, /*include_orphanage=*/false)) {
                m_txpackagetracker->TransactionRejected({gtxid.GetHash()});
            } else {
