...
private:
    ...
    */
    ...
    void AddOrphanResolutionCandidate(NodeId nodeid, const uint256& orphan_wtxid, std::chrono::microseconds current_time)
        EXCLUSIVE_LOCKS_REQUIRED(::cs_main);
    ...
    /** Get NodeIds of every peer that knows this transaction. */
    std::vector<NodeId> GetAnnouncers(const uint256& txid, const uint256& wtxid) EXCLUSIVE_LOCKS_REQUIRED(m_peer_mutex);
    ...
    std::optional<std::vector<uint256>> MaybeGetAncPkgInfo(CNode& pfrom, Peer& peer, const CTransactionRef& tx);
...
void PeerManagerImpl::AddOrphanResolutionCandidate(NodeId nodeid, const uint256& orphan_wtxid, std::chrono::microseconds current_time)
{
    ...
    if (!relay_permissions) {
        const auto packages_in_flight{m_txpackagetracker ? m_txpackagetracker->CountInFlight(nodeid) : 0};
        const auto packages_total{m_txpackagetracker ? m_txpackagetracker->Count(nodeid) : 0};
        if (packages_in_flight >= MAX_IN_FLIGHT_PACKAGES ||
            packages_total + m_txrequest.Count(nodeid) >= MAX_PEER_TX_ANNOUNCEMENTS) {
    ...
    if (!GetPeerRef(nodeid)->m_package_relay && m_package_relay_peers > 0) delay += TXID_RELAY_DELAY;
...
void PeerManagerImpl::BlockConnected(const std::shared_ptr<const CBlock>& pblock, const CBlockIndex* pindex)
{
    ...
    if (m_txpackagetracker) {
        ...
        for (const CTransactionRef& ptx : pblock->vtx) block_wtxids.insert(ptx->GetWitnessHash());
        m_txpackagetracker->FinalizeTransactions(block_wtxids, {});
    }
    m_last_tip_update = GetTime<std::chrono::seconds>();
...
std::vector<NodeId> PeerManagerImpl::GetAnnouncers(const uint256& txid, const uint256& wtxid)
{
    std::vector<NodeId> announcers;
    for (auto [nodeid, peer] : m_peer_map) {
        auto tx_relay = peer->GetTxRelay();
        if (!tx_relay) continue;
        if (tx_relay->m_tx_inventory_known_filter.contains(txid) ||
            tx_relay->m_tx_inventory_known_filter.contains(wtxid)) {
            announcers.push_back(nodeid);
        }
    }
    return announcers;
}
...
std::optional<std::vector<uint256>> PeerManagerImpl::MaybeGetAncPkgInfo(CNode& pfrom, Peer& peer, const CTransactionRef& tx)
{
    if (!peer.m_package_relay) {
        LogPrintf("Disconnecting peer %d -- requested ancestor package info but not allowed\n", pfrom.GetId());
        pfrom.fDisconnect = true;
        return std::nullopt;
    }
    ...
    while (it != peer.m_getdata_requests.end() && (it->IsGenTxMsg() || it->IsMsgAncPkgInfo())) {
        ...
        if (tx) {
            if (inv.IsGenTxMsg()) {
                ...
                auto ancestor_wtxids = MaybeGetAncPkgInfo(pfrom, peer, tx);
                if (ancestor_wtxids == std::nullopt) {
                    // This peer is going to be disconnected, so there's no need to
                    // process the other requests.
                    break;
    ...
    if (msg_type == NetMsgType::WTXIDRELAY) {
        ...
        if (pfrom.GetCommonVersion() >= WTXID_RELAY_VERSION) {
            if (!peer->m_wtxid_relay) {
                ...
                m_wtxid_relay_peers++;
                if (m_enable_package_relay) m_txpackagetracker->ReceivedWtxidRelay(peer->m_id);
            } else {
    ...
    if (msg_type == NetMsgType::INV) {
        ...
        for (CInv& inv : vInv) {
            ...
            if (inv.IsMsgBlk()) {
                ...
                if (inv.IsMsgWtx() && m_orphanage.HaveTx(gtxid) && !m_chainman.ActiveChainstate().IsInitialBlockDownload()) {
    ...
    if (msg_type == NetMsgType::ANCPKGINFO) {
        ...
        if (!pkgtxns_to_request.empty()) {
            ...
            m_txpackagetracker->ReceivedAncPkgInfo(pfrom.GetId(), rep_wtxid, txdata_status, pkgtxns_to_request,
                                                   total_orphan_size, current_time + GETDATA_TX_INTERVAL);
    ...
    if (msg_type == NetMsgType::TX) {
        ...
        else if (state.GetResult() == TxValidationResult::TX_MISSING_INPUTS)
        {
            ...
            if (!fRejectedParents) {
                ...
                const auto current_time{GetTime<std::chrono::microseconds>()};
                for (const auto nodeid : GetAnnouncers(tx.GetHash(), tx.GetWitnessHash())) {
