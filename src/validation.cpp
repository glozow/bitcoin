...
PackageMempoolAcceptResult MemPoolAccept::AcceptMultipleTransactions(const std::vector<CTransactionRef>& txns, ATMPArgs& args)
{
    ...
    if (!args.m_test_accept && !IsChildWithParents(txns, /* exact */ true)) {
        // Unless this is a testmempoolaccept, we only allow packages that consist of exactly one
        // child and all of its parents. We can then think of this package as one big transaction
        // with inputs = inputs of parents, outputs = outputs of child + unspent outputs of parents
