...
private:
    ...
    bool CheckFeeRate(size_t package_size, CAmount package_fee, TxValidationState& state) EXCLUSIVE_LOCKS_REQUIRED(::cs_main, m_pool.cs)
    {
        ...
        if (mempoolRejectFee > 0 && package_fee < mempoolRejectFee) {
            return state.Invalid(TxValidationResult::TX_SINGLE_FAILURE, "mempool min fee not met", strprintf("%d < %d", package_fee, mempoolRejectFee));
        }

        if (package_fee < m_pool.m_min_relay_feerate.GetFee(package_size)) {
            return state.Invalid(TxValidationResult::TX_SINGLE_FAILURE, "min relay fee not met",
...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    if (!bypass_limits && ws.m_modified_fees < m_pool.m_min_relay_feerate.GetFee(ws.m_vsize)) {
        ...
        // TX_SINGLE_FAILURE, because it cannot be bypassed using package validation.
...
PackageMempoolAcceptResult MemPoolAccept::AcceptPackage(const Package& package, ATMPArgs& args)
{
    ...
    const auto& child_wtxid = linearized_package.Txns().back()->GetWitnessHash();
    Assume(IsPackageWellFormed(linearized_package.Txns(), package_state_quit_early, /*require_sorted=*/true));
    Assume(child_wtxid == package.back()->GetWitnessHash());
        ...
        if (m_pool.exists(GenTxid::Wtxid(wtxid))) {
            ...
            if (PreChecks(single_args, ws)) {
                ...
                case TxValidationResult::TX_MISSING_INPUTS: case TxValidationResult::TX_CONFLICT:
                {
                    ...
                    //
                    // Add this failure to results_final since we won't be trying to
                    // validate it again.
                    linearized_package.SkipWithDescendants(tx);
                    // Add to results_final because we will not try to validate this transaction again.
                    results_final.emplace(wtxid, MempoolAcceptResult::Failure(ws.m_state));
                    break;
                }
    ...
    if (!quit_early) {
        ...
        for (const auto& tx : linearized_package.FilteredTxns()) {
            ...
            // Never skip the last transaction, as we want to return a validation result for this transaction.
            if (tx->GetWitnessHash() != child_wtxid && subpackage_fee_vsize.has_value()) {
                TxValidationState placeholder_state;
                if (!CheckFeeRate(subpackage_fee_vsize->second, subpackage_fee_vsize->first, placeholder_state)) {
            ...
            for (const auto& subpackage_tx : subpackage.value()) {
                ...
                if (subpackage_it != subpackage_result.m_tx_results.end()) {
                    ...
                    // FilteredTxns(). A tx is also never validated more than once in this loop:
                    // - if a transaction succeeds, it is skipped in subsequent iterations
                    // - if a transaction fails, we exit the loop immediately
            ...
            if (subpackage_result.m_state.IsInvalid()) {
                // Abort if any failures are encountered. This means we may skip validation for
                // some transactions entirely, including ones we potentially should have accepted.
                // We expect that some other peer will announce those transactions to us.
                package_state_quit_early = subpackage_result.m_state;
                break;
            }
        }
    }

    auto submission_result = PackageMempoolAcceptResult(package_state_quit_early, {});
    ...
    for (const auto& tx : package) {
        ...
        if (submission_result.m_tx_results.count(wtxid) > 0) {
            ...
            submission_result.m_tx_results.emplace(wtxid, it->second);
            ...
            // Backfill with "unknown result."
            TxValidationState unknown_state;
            unknown_state.Invalid(TxValidationResult::TX_UNKNOWN, "unknown-not-validated");
            submission_result.m_tx_results.emplace(wtxid, MempoolAcceptResult::Failure(unknown_state));
        }
    }
