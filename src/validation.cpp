...
private:
    ...
    // Run mempool ancestor/descendant limit and BIP125 RBF Checks. Note that this does not
    // constitute all mempool checks.
...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    for (const CTxIn &txin : tx.vin)
    {
        ...
        if (ptxConflicting) {
            ...
            if (!m_txid_conflicts.count(ptxConflicting->GetHash()))
            {
                if (IsRBFOptOut(*ptxConflicting)) {
                    return state.Invalid(TxValidationResult::TX_MEMPOOL_POLICY, "txn-mempool-conflict");
                }

                m_txid_conflicts.insert(ptxConflicting->GetHash());
            }
        }
    }
...
bool MemPoolAccept::MempoolChecks(Workspace& ws)
{
    ...
    if (!m_pool.CalculateMemPoolAncestors(*entry, setAncestors, m_limit_ancestors, m_limit_ancestor_size, ws.m_tx_limit_descendants, ws.m_tx_limit_descendant_size, errString)) {
        setAncestors.clear();
        ...
        if (nSize >  EXTRA_DESCENDANT_TX_SIZE_LIMIT ||
                !m_pool.CalculateMemPoolAncestors(*entry, setAncestors, 2, m_limit_ancestor_size, ws.m_tx_limit_descendants + 1, ws.m_tx_limit_descendant_size + EXTRA_DESCENDANT_TX_SIZE_LIMIT, dummy_err_string)) {
    ...
    // pathological case by making sure conflicts and ancestors don't intersect.
    if (!SpendsAndConflictsDisjoint(m_collective_ancestors, m_txid_conflicts, state, hash)) return false;

    // If we don't hold the lock we might not get all conflicts; the
    // subsequent RemoveStaged() and addUnchecked() calls don't guarantee
    // mempool consistency for us.
    CFeeRate newFeeRate(ws.m_modified_fees, nSize);
    if (!PaysMoreThanConflicts(m_iter_conflicts, newFeeRate, state, hash)) return false;

    // Calculate all conflicting entries and enforce Rules 2 and 5.
    if (!GetEntriesForRBF(tx, m_pool, m_iter_conflicts, state, m_all_conflicts)) return false;
    if (!HasNoNewUnconfirmed(tx, m_pool, m_iter_conflicts, state)) return false;
    ...
    if (!PaysForRBF(m_conflicting_fees, m_conflicting_size, ws.m_modified_fees,
...
bool MemPoolAccept::Finalize(const ATMPArgs& args, Workspace& ws)
{
    ...
    if (m_all_conflicts.size() > 0) {
        std::string removed_txids;
        for (const auto& it : m_all_conflicts) {
                                    ...
                                    std::map<const uint256, const MempoolAcceptResult>& results)
{
    ...
    // Find the wtxids of the transactions that made it into the mempool.
    std::set<uint256> submitted_package_wtxids;
    for (Workspace& ws : workspaces) {
        auto wtxid = ws.m_ptx->GetWitnessHash();
        if (m_pool.exists(wtxid)) submitted_package_wtxids.insert(wtxid);
    }

    if (submitted_package_wtxids.size() != workspaces.size()) {
        // Not all of the transactions made it into the mempool. Pull them all out.
        CTxMemPool::setEntries submitted_package_entries = m_pool.GetIterSet(submitted_package_wtxids);
        // Use updateDescendants=false because it is not possible for any of the package
        // transactions to have descendants in the mempool that aren't part of the package.
        m_pool.RemoveStaged(submitted_package_entries, false, MemPoolRemovalReason::SIZELIMIT);
        for (Workspace& ws : workspaces) {
...
PackageMempoolAcceptResult MemPoolAccept::AcceptMultipleTransactions(const std::vector<CTransactionRef>& txns, ATMPArgs& args)
{
    ...
    if (!args.m_test_accept && !IsChildWithParents(txns)) {
        // Unless this is a testmempoolaccept, we only allow packages that consist of
        // exactly one child and its parents.
    ...
    if (IsChildWithParents(txns) && !CheckFeeRate(total_size, total_modified_fees, placeholder_state)) {
                                                   ...
                                                   const Package& package, bool test_accept)
{
    ...
    auto args = test_accept
              ? MemPoolAccept::ATMPArgs::PackageTestAccept(chainparams, GetTime(), coins_to_uncache)
              : MemPoolAccept::ATMPArgs::PackageChildWithParents(chainparams, GetTime(), coins_to_uncache);
