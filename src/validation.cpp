...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    if (!m_pool.CalculateMemPoolAncestors(*entry, ws.m_ancestors, m_limit_ancestors, m_limit_ancestor_size,
        ...
        if (ws.m_vsize > EXTRA_DESCENDANT_TX_SIZE_LIMIT ||
                !m_pool.CalculateMemPoolAncestors(*entry, ws.m_ancestors, 2, m_limit_ancestor_size,
                                                  ws.m_tx_limit_descendants + 1,
                                                  ws.m_tx_limit_descendant_size + EXTRA_DESCENDANT_TX_SIZE_LIMIT,
                                                  dummy_err_string)) {
                                         ...
                                         PackageValidationState& package_state)
{
    ...
    // Recheck signaling requirements. There are three acceptable conditions:
    // 1. user configured full rbf
    // 2. all directly conflicting transactions signal BIP125 replaceability
    // 3. all directly conflicting and replacement transactions are V3
    if (!m_pool.m_full_rbf &&
        !std::all_of(direct_conflict_iters.begin(), direct_conflict_iters.end(),
                     [](const auto& iter) { return SignalsOptInRBF(iter->GetTx()); } )) {
        if (const auto err_string{CanReplaceV3(direct_conflict_iters, txns)}) {
            return package_state.Invalid(PackageValidationResult::PCKG_POLICY,
                                         "package RBF failed: signaling", *err_string);
        }
    }
