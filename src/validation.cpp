...
private:
    ...
    {
        m_total_modified_fees = 0;
        m_total_vsize = 0;
        m_rbf = false;
        m_all_conflicts.clear();
        m_replaced_transactions.clear();
                                  ...
                                  std::map<uint256, MempoolAcceptResult>& results)
{
    ...
    if (!m_replaced_transactions.empty()) {
        LogPrint(BCLog::MEMPOOL, "replaced %u mempool transactions with %u new one(s) for %s additional fees, %d delta bytes\n",
                 m_replaced_transactions.size(), workspaces.size(),
                 m_total_modified_fees - static_cast<int>(m_conflicting_fees),
                 m_total_vsize - static_cast<int>(m_conflicting_size));
    }
...
MempoolAcceptResult MemPoolAccept::AcceptSingleTransaction(const CTransactionRef& ptx, ATMPArgs& args)
{
    ...
    if (args.m_test_accept) {
        return MempoolAcceptResult::Success(std::move(m_replaced_transactions), ws.m_vsize,
    ...
    if (!m_replaced_transactions.empty()) {
        LogPrint(BCLog::MEMPOOL, "replaced %u mempool transactions with 1 new transaction for %s additional fees, %d delta bytes\n",
                 ...
                 ws.m_modified_fees - static_cast<int>(m_conflicting_fees),
                 ws.m_vsize - static_cast<int>(m_conflicting_size));
    }
