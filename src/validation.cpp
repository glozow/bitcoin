...
public:
    ...
    struct ATMPArgs {
         ...
         * partially submitted.
         */
        const bool m_atomic_package_submission{false};
        ...
        static ATMPArgs PackageTestAccept(const CChainParams& chainparams, int64_t accept_time,
            ...
            return ATMPArgs{/* m_chainparams */ chainparams,
                            ...
                            /* m_atomic_package_submission */ false,
        ...
        static ATMPArgs PackageChildWithParents(const CChainParams& chainparams, int64_t accept_time,
            ...
            return ATMPArgs{/* m_chainparams */ chainparams,
                            ...
                            /* m_test_accept */ false,
                            /* m_allow_bip125_replacement */ false,
                            /* m_atomic_package_submission */ true,
                            /* m_package_feerates */ true,
            };
        }
    };
...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    if (ws.m_replacement_transaction)
    {
        ...
        if (ws.m_modified_fees < ws.m_conflicting_fees)
        {
            return state.Invalid(TxValidationResult::TX_MEMPOOL_POLICY, "insufficient fee",
                    strprintf("rejecting replacement %s, less fees than conflicting txs; %s < %s",
                        hash.ToString(), FormatMoney(ws.m_modified_fees), FormatMoney(ws.m_conflicting_fees)));
        }

        // Finally in addition to paying more fees than the conflicts the
        // new transaction must pay for its own bandwidth.
        CAmount nDeltaFees = ws.m_modified_fees - ws.m_conflicting_fees;
...
bool MemPoolAccept::Finalize(const ATMPArgs& args, Workspace& ws)
{
    ...
    if (!args.m_atomic_package_submission && !bypass_limits) {
                                    ...
                                    std::map<const uint256, const MempoolAcceptResult>& results)
{
    ...
    for (Workspace& ws : workspaces) {
        ...
        // Submission of a package to mempool should be atomic: either all of the transactions are
        // submitted or there are no changes to the mempool. Do this by allowing the mempool to
        // temporarily bypass limits while submitting transactions individually, then trimming at
        // the very end.
        if (!Finalize(args, ws)) {
            results.emplace(ws.m_ptx->GetWitnessHash(), MempoolAcceptResult::Failure(ws.m_state));
            return package_state.Invalid(PackageValidationResult::PCKG_TX_POLICY, "transaction failed");
