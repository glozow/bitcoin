...
bool TxOrphanage::AddTx(const CTransactionRef& tx, NodeId peer)
{
    ...
    if (m_orphans.count(hash)) {
        if (m_wtxid_to_orphan_it.count(wtxid) == 0) {
            LogPrint(BCLog::TXPACKAGES, "same txid (%s) different witness (%s) already in orphanage\n", hash.ToString(), wtxid.ToString());
        }
        return false;
    }
    ...
    LogPrint(BCLog::TXPACKAGES, "stored orphan tx %s (wtxid=%s) txsize=%u, (mapsz %u outsz %u)\n", hash.ToString(), wtxid.ToString(), sz,
...
int TxOrphanage::EraseTxNoLock(const Txid& txid)
{
    ...
    LogPrint(BCLog::TXPACKAGES, "   removed orphan tx %s (wtxid=%s) after %d in orphanage\n", txid.ToString(), wtxid.ToString(),
             nNow + ORPHAN_TX_EXPIRE_TIME - it->second.nTimeExpire);
...
void TxOrphanage::EraseForPeer(NodeId peer)
{
    ...
    if (nErased > 0) LogPrint(BCLog::TXPACKAGES, "Erased %d orphan tx from peer=%d due to @@DISCONNECT@@\n", nErased, peer);
}
...
std::vector<std::pair<CTransactionRef, NodeId>> TxOrphanage::GetChildren(const CTransactionRef& parent) const
{
    LOCK(m_mutex);
