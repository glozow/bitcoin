        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    for (const auto& tx : txns) {
        ...
        } else if (tx_result.m_result_type == MempoolAcceptResult::ResultType::MEMPOOL_INFO) {
            // Transaction was already in the mempool. Treat this like a successful validation
            // result, but don't check for max-fee-exceeded because it's already in the mempool.
            CHECK_NONFATAL(txns.size() > 1);
            result_inner.pushKV("allowed", true);
            result_inner.pushKV("vsize", tx_result.m_vsize.value());
            UniValue fees(UniValue::VOBJ);
