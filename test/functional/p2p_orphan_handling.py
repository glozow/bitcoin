...
class OrphanHandlingTest(BitcoinTestFramework):
    ...
    def test_orphan_rejected_parents_exceptions(self):
        ...
        # Relay the child. It should not be accepted because it has missing inputs. It should be added to orphanage.
        self.relay_transaction(peer2, child_nonsegwit["tx"])
        assert child_nonsegwit["txid"] not in node.getrawmempool()
        assert tx_in_orphanage(node, child_nonsegwit["tx"])

        self.nodes[0].bumpmocktime(GETDATA_TX_INTERVAL)
        peer1.assert_never_requested(int(parent_other["txid"], 16))
        # This parent is requested
        peer2.wait_for_getdata([int(parent_other["txid"], 16)])
        # This parent is not requested because its hash (txid == wtxid) has been added to the rejection filter.
