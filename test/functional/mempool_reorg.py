...
class MempoolCoinbaseTest(BitcoinTestFramework):
    ...
    def test_reorg_relay(self):
        ...
        # node1 has a longer chain in which tx_disconnected was not confirmed.
        self.generate(self.nodes[0], 3, sync_fun=self.no_op)
        self.generate(self.nodes[1], 1, sync_fun=self.no_op)

        # Create a transaction and submit it to node0's mempool.
        tx_before_reorg = self.wallet.send_self_transfer(from_node=self.nodes[1])

        # Create a child of that transaction and submit it to node0's mempool.
        ...
        # Reconnect the nodes and sync chains. Node1's chain should win.
        self.connect_nodes(0, 1)
        self.sync_blocks()
        ...
        with p2p_lock:
            assert_equal(peer1.last_message["tx"].tx.getwtxid(), tx_after_reorg["tx"].getwtxid())
            # assertion commented out, node1 shouldn't send the requested tx.
