...
# Time after which the node is likely to have announced a transaction after accepting it to mempool.
# The timer is randomly set, so there is a (very small) chance it takes longer.
TX_ANNOUNCEMENT_DELAY = 60
...
SUPPORTED_PKG_RELAY = PKG_RELAY_PKGTXNS | PKG_RELAY_ANCPKG
...
class PackageRelayer(P2PTxInvStore):
    ...
    def on_version(self, message):
        ...
        if self._send_sendpackages:
            ...
            sendpackages_message.versions = SUPPORTED_PKG_RELAY
...
class PackageRelayTest(BitcoinTestFramework):
    ...
    def test_sendpackages(self):
        ...
        assert_equal(peer_normal.sendpackages_received, [SUPPORTED_PKG_RELAY])
        ...
        assert_equal(node.getpeerinfo()[0]["bytesrecv_per_msg"]["sendpackages"], 32)
        assert_equal(node.getpeerinfo()[0]["bytessent_per_msg"]["sendpackages"], 32)
        assert_equal(peer_no_wtxidrelay.sendpackages_received, [SUPPORTED_PKG_RELAY])
        ...
        assert_equal(peer_no_sendpackages.sendpackages_received, [SUPPORTED_PKG_RELAY])
        assert not node.getpeerinfo()[0]["relaytxpackages"]
        node.disconnect_p2ps()
    ...
    def test_ancpkginfo_requests(self):
        ...
        node.bumpmocktime(TX_ANNOUNCEMENT_DELAY)
        peer_info_requester.send_and_ping(tx_request)
    ...
    def test_orphan_get_ancpkginfo(self):
        ...
        node.bumpmocktime(NONPREF_PEER_TX_DELAY + 1)
    ...
    def test_orphan_handling_prefer_outbound(self):
        ...
        node.bumpmocktime(ORPHAN_ANCESTOR_GETDATA_INTERVAL)
    ...
    def test_pkgtxns(self):
        ...
        assert_equal(len(peer_requester_unannounced.get_invs()), 0)
        # FIXME: failing, node sends pkgtxns
