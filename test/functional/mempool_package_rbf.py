...
class PackageRBFTest(BitcoinTestFramework):
    ...
    def test_package_rbf_signaling(self):
        ...
        self.log.info("Test that original transactions must signal replaceability using V3, even if they signal BIP125")
        coin = self.coins[0]
        del self.coins[0]
        ...
        package_hex, package_txns = self.create_simple_package(parent_coins=[coin], parent_fee=0, child_fee=DEFAULT_FEE * 3)
        assert all([tx.nVersion == 3 for tx in package_txns])
        assert_raises_rpc_error(-25, "package RBF failed: V3 signaling required", node.submitpackage, package_hex)
        self.assert_mempool_contents(expected=[tx_bip125_v2["tx"]], unexpected=package_txns)
        self.generate(node, 1)
